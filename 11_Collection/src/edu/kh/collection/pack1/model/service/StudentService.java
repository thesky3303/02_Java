	...
	*/
	...
	public String addStudent() throws InputMismatchException{
		...
		if( studentList.add(new Student(name, age, region, gender, score)) ) {
			...
			// (반환형)              -> 제너릭 <Student> 때문에 List에 추가할 수 있는 객체 타입이
			//                          Student로 제한됨
	...
	public String updateStudent() throws InputMismatchException{
		...
		// 3) studentList 범위 내 인덱스번호인지 검사
			...
			// 입력받은 index번째에 새로운 학생정보를 세팅 -> 수정
			// index 번째에 있던 기존 학생 정보가 반환된고, 그 객체를 temp 에 저장
	...
	public void remove() throws InputMismatchException {
		...
		if(studentList.isEmpty()) { 
			...
			if (ch == 'Y') {
				Student temp = studentList.set(index, new Student());
				System.out.print(temp.getName() +"의 정보가 삭제되었습니다");
	 ...
	 * 문자열 입력받아 studentList에서 꺼내온 student 객체의 aname갑에 포함되는 문자열인지 검사
	...
	public void searchName2() {
		...
		for(Student std : studentList) {
			// 순서 유의!
			// boolean String.contains(문자열) : String에 문자열이 포함되어있으면 truq / 없으면 false
